...
public sealed class VendorImportControllerTests
{
    ...
    public async Task ImportReviews_imports_valid_rows_and_skips_missing_vendor()
    {
        ...
        vendorRepo.Setup(r => r.GetByIdAsync(existingVendorId))
            .ReturnsAsync(new Vendor { Id = existingVendorId, Name = "Vendor A" });
        vendorRepo.Setup(r => r.GetByIdAsync(It.Is<Guid>(g => g != existingVendorId)))
        ...
        var result = await controller.ImportReviews(new List<VendorReviewImportRequest>
        {
            new() { VendorId = existingVendorId, AuthorDisplayName = "Jane Referrer", ReviewText = "Great service." },
            new() { VendorId = Guid.NewGuid(), AuthorDisplayName = "Missing", ReviewText = "Will skip." }
        });
        ...
        reviewRepo.Verify(r => r.UpsertAsync(It.Is<VendorReview>(v =>
            ...
            && VendorReviewTimestamps.IsUnknown(v.ModifiedUtc))), Times.Once);
    ...
    public async Task ImportReviews_skips_null_rows_without_throwing()
    {
        ...
        {
            null!,
            new() { VendorId = existingVendorId, AuthorDisplayName = "R", ReviewText = "Kept." }
        });
    ...
    public async Task ImportVendorsAndReviews_imports_vendor_and_review_by_fingerprint()
    {
        ...
        reviewRepo.Setup(r => r.UpsertAsync(It.IsAny<VendorReview>()))
            .ReturnsAsync((VendorReview r) => r);
        youthRepo.Setup(r => r.UpsertAsync(It.IsAny<YouthServiceListing>()))
            .ReturnsAsync((YouthServiceListing y) => y);
        auditRepo.Setup(r => r.AddAsync(It.IsAny<NewAuditLogEntry>()))
            .Returns(Task.CompletedTask);
        ...
        var ok = Assert.IsType<OkObjectResult>(result);
        Assert.NotNull(ok.Value);
        vendorRepo.Verify(r => r.UpsertAsync(It.Is<Vendor>(v => v.Name == "Vendor A" && v.Notes == "Imported notes")), Times.Once);
        reviewRepo.Verify(r => r.UpsertAsync(It.Is<VendorReview>(v =>
    ...
    public async Task ImportVendorsAndReviews_skips_duplicate_vendor_fingerprints()
    {
        ...
        var result = await controller.ImportVendorsAndReviews(new VendorSeedImportRequest
        {
            ...
            Reviews = new List<VendorSeedImportReview>
            {
                new() { VendorFingerprint = "dup-1", AuthorDisplayName = "Referrer", ReviewText = "Great." }
            }
        });
    ...
    {
        var controller = new VendorImportController(vendorRepository, reviewRepository, youthServiceListingRepository, userRepository, auditLogRepository)
        {
