...
- **`package-lock.json` is generated by npm 11**, and npm 10 - which Node 22 currently bundles -
  rejects it: `npm ci` fails with `Missing: chokidar@4.0.3 from lock file`. The two majors disagree
  about which transitive dependencies belong in the lock, and the disagreement is mutual (a lock
  written by npm 10 fails under npm 11, missing optional platform binaries). Both workflows pin
  `npm@11` before `npm ci` for this reason. If you regenerate the lock, do it on npm 11, or change
  the pin to match. `dotnet publish` uses `npm install`, which tolerates the drift - which is why
  this went unnoticed until the frontend first ran in CI.
...
- `Dues:ZelleEmail` (env var `Dues__ZelleEmail`, or `dotnet user-secrets` in the `Web` project) is the Zelle recipient address the Dues page shows under "Pay Without Fees". It is a personal Gmail mailbox, so **never commit it** - `appsettings.json` holds only an empty placeholder and `appsettings.MockData.json` a fake `@cohad.local` address. The SPA reads it from `GET api/payment/options` (Resident-gated, same as the rest of `PaymentController`); when it is blank the endpoint returns `null` and the Zelle note is not rendered at all.
