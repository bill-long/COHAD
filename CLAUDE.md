...
- **Scheduled maintenance jobs (user purge, PayPal sync):** Both run **in-process** as hosted services in `Web` (they were previously timer-triggered Azure Functions in a separate Function App; that project is gone). Each self-disables when its flag is off - `UserPurge:Enabled` / `PayPal:SyncEnabled`. The two pace themselves differently, on purpose:
  ...
  - A **dry sweep paces on its own key** (`user-purge-dryrun`). Sharing the live key would either let a dry run consume the live interval, delaying real purges, or let a live run silence the dry run the cut-over runbook tells the operator to verify with.
  - `UserPurgeRunner` writes the audit entry **before** the delete, so an irreversible deletion can never go unrecorded; a delete that then fails gets a second entry recording *uncertainty* (a write that throws may still have committed, and a deleted user is never a candidate again, so a flat denial could never be corrected). Because that ordering writes entries for deletions that may not happen, a run **aborts after `MaxConsecutiveDeleteFailures` (3)** consecutive delete failures rather than writing two rows per candidate for the whole batch on every sweep.
  - Both jobs treat a **future-dated pacing stamp** (clock skew, or a state document restored from another environment) as fully elapsed and warn. Without that, every comparison goes negative and the job latches off silently - forever, with nothing logged.
...
- The `BackgroundJobState` Cosmos container must exist before enabling `UserPurge:Enabled` or `PayPal:SyncEnabled` (non-partitioned, `/NoPartitionKey`). It is needed for a `DryRun` purge too, which paces on its own key in the same container. Without it the PayPal sync fails fast on every tick rather than silently re-running.
