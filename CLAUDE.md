...
- The **`UnsubscribeLink` Cosmos container must exist** before sending mail in a new environment (non-partitioned, `/NoPartitionKey`, container TTL ~400 days), like every other container here. Without it, issuing a link fails and the recipient is marked `Failed` and retried - deliberately, because the alternative is sending bulk mail with no unsubscribe mechanism at all. The TTL only prunes; the credential's own 365-day lifetime is enforced in code (`UnsubscribeLink.MaxLinkAge`), so a container created without a TTL does not turn links into permanent credentials.
