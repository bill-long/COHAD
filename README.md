...
5. **Zelle on the Dues page** is driven by the `Dues__ZelleEmail` app setting. The address is a personal mailbox, so it is deliberately absent from source; `appsettings.json` carries only an empty placeholder. Set it and the "Pay Without Fees" panel gains Zelle instructions (send to that address; the recipient shows as "Canyon Oaks Homeowners Association"). Leave it blank and the panel shows check/cash only.
6. **Scheduled jobs** (user purge, PayPal sync) run in-process as hosted services in the `Web` app - there is nothing separate to deploy. Both are off by default; enable via `UserPurge__Enabled` / `PayPal__SyncEnabled`. The PayPal sync requires a **`BackgroundJobState`** Cosmos container (non-partitioned, `/NoPartitionKey`), provisioned out-of-band like every other container, which is what paces it across restarts; the purge needs no state. **Always On** must be enabled on the host, or the app unloads when idle and the timers never fire.
